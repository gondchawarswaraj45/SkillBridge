    ...
    private Button createNavLink(String title, boolean active, Runnable action) {
        ...
        if (active) {
            btn.setStyle("-fx-background-color: " + AppTheme.COLOR_PRIMARY
                    + "; -fx-text-fill: white; -fx-background-radius: 6; -fx-padding: 6 12; -fx-cursor: hand;");
        } else {
            btn.setStyle("-fx-background-color: transparent; -fx-text-fill: "
                    + (AppTheme.isDarkMode() ? "#CBD5E1" : "#1E293B") + "; -fx-padding: 6 12; -fx-cursor: hand;");
            btn.setOnMouseEntered(e -> btn.setStyle("-fx-background-color: "
                    + (AppTheme.isDarkMode() ? "#1E293B" : "#CAD8EA") + "; -fx-text-fill: " + AppTheme.COLOR_PRIMARY
